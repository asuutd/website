...
const LoginForm = ({
	...
	const filteredProviders = useMemo(() => {
		...
		return Object.values(providers).filter(
			(provider) => !['Credentials', 'Email'].includes(provider.name)
		);
	}, [providers]);
	...
	return (
		<div className="flex w-full sm:max-w-xl bg-white rounded-lg p-8 flex-col">
			...
			<form className="form-control" onSubmit={handleEmailLogin}>
				<label className="label" htmlFor="input-email-for-email-provider">
					Email
				</label>
				<input
					...
					required
				/>
				<button className="btn btn-outline my-2" type="submit">
					Sign in with Email
				</button>
			</form>
		</div>
