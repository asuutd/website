...
import { CustomRadioGroupField } from '@/utils/forms';
import { FormContext } from '../Creator';
import { ArrayElement } from '@/utils/misc';

const RadioGroupView = ({ id, data }: { id: number; data: CustomRadioGroupField }) => {
	const { edit, deleteElement } = useContext(FormContext);
	const [temporaryEdit, setTemporaryEdit] = useState<CustomRadioGroupField>(structuredClone(data));
