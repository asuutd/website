        ...
        <div>
          ...
          <div className="space-y-4">
            {topFamilies.map((family, index) => (
              <FamilyCard key={`top-family-${family.id}`} name={family.family_name} position={index + 1} points={family.score ?? -1} member_names={family.members.map((member) => (member as unknown as Member).jonze_name)} />
            ))}
          </div>
        </div>
        <div>
      ...
      <div className="mt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {pointEarningMethods.map((method, index) => (
            <PointEarningMethodCard key={`point-earning-method-${index}`} title={method.title} description={method.description} />
          ))}
        </div>
      </div>
    </div>
...
function MemberCard(props: { memberName: string, position: number, points: number, familyName: string }) {
...
function FamilyCard(props: { name: string, position: number, points: number, member_names: string[] }) {
      ...
      <div className="flex items-center gap-2">
        {member_names.map((member_name, index) => (
          <Avatar key={`${name}-${member_name}`}>
            <AvatarFallback>
              {member_name.split(/(?:\s|-)+/).map((name) => name.slice(0, 1)).join('').toUpperCase()}
            </AvatarFallback>
          </Avatar>
        ))}
      </div>
