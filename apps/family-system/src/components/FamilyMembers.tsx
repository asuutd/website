...
function FamilyMembers() {
    ...
    useEffect(() => {
        ...
        const run = async () => {
            const res = await fetch(`/api/members-by-family-tag?tag=${encodeURIComponent(jonze_family_tag)}`)
            if (!res.ok) {
                return
            }
        ...
        <div>
            ...
            <select name="memberId" onChange={(e)=>setMemberId(e.target.value)} value={memberId}>
                ...
                {allMembers.map((member: Member) => (
                    <option key={member.jonze_member_id} value={member.jonze_member_id}>{member.jonze_name}</option>
                ))}
            </select>
